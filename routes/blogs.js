...
module.exports = (router) => {
      ...
      if(!req.body.title) {
          ...
          if(!req.body.body) {
            ...
            if(!req.body.createdBy) {
                res.json({ success: false, message: 'Blog creater name is required'});
            } else {
                const blog = new Blog({
    ...
    router.get('/allBlogs', (req,res) => {
        ...
        }).sort({ '_id': -1 })//to reverse the order of the resulting documents so that new ones will be on the top.
    })  
                        ...
                        if(err) {
                            ...
                            if(!user) {
                                ...
                                if(user.username !== blog.createdBy) {
                                    ...
                                    res.json({ success: false, message: 'You are not authorized to edit the post'});
                                } else {
    ...
    if(!req.body._id) {
        ...
        Blog.findOne({ _id: req.body._id }, (err, blog) => {
            if(err) {
                ...
                if(!blog) {
                  res.json({ success: false, message: 'Blog ID was not foun d'})
                } else {
                    User.findOne({ _id: req.decoded.userId }, (err, user) => {
                        if(err) {
                            res.json({ success: false, message: err });
